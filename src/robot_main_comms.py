...
class Patroller():
    ...
    def __init__(self):
        ...
        #Stuff for messaging from robot > server
    ...
    def state_at_waypoint(self):
        ...
        ts = Tag_scan(self.start_time,self.reward_csv_path,self.name) #calls tag scan, does the thing, continues
        ts.tag_scan()
...
class Tag_scan(): #==============================================================================
    ...
    def __init__(self,start_time,reward_csv_path,name):
        self.ID_list = []
        self.buffer = []
        ...
        self.server_callback_wait = True
        ...
        self.server_pub = rospy.Publisher('/server_pub', String, queue_size=10) #MIGHT NEED TO MAKE THIS A SEND AND WAIT FOR RESPONSE THING >
        self.server_sub = rospy.Subscriber('/server_sub', String, self.server_callback)

        request_message = {
            'name':self.name,
            'request':True,
        }
         # when initialising (at a waypoint), send a request to the server
        rospy.loginfo("Sent request for belief update")
        rospy.loginfo("Waiting for response...")
        # to update beliefs
        r = rospy.Rate(1)
        while self.server_callback_wait:
            self.server_pub.publish(json.dumps(request_message))
            r.sleep()
    ...
    def server_callback(self,msg):
        global reward_ID_seen
        global no_reward_ID_seen
        #callback for response from server, update beliefs to global belief
        data = json.loads(msg.data)
        print(f"Data Received: {data}")
        reward_ID_seen = data['rewards']
        no_reward_ID_seen = data['no_rewards']
